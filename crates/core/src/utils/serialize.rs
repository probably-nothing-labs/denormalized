...
// pub struct SerializableScalarValue {
//     value: ScalarValue,
// }

// impl Serialize for SerializableScalarValue {
...
//                                 .map(|i| ScalarValue::try_from_array(&values, i).map(Some).unwrap())
...
//                     st.serialize_field("value", &Option::<Vec<Option<ScalarValue>>>::None)?;
//                 }
//                 st.end()
//             }
...
//                 st.serialize_field("value", &v.map(|x| (x.months, x.days, x.nanoseconds)))?;
//                 st.end()
//             }
...
//                     &fields
//                         .iter()
//                         .map(|(i, f)| (i, f.name().to_string(), f.data_type().to_string()))
//                         .collect::<Vec<_>>(),
//                 )?;
//                 st.serialize_field(
//                     "mode",
...
//                         st.serialize_field("value", &nested_values)?;
//                     } else {
//                         return Err(serde::ser::Error::custom("Invalid LargeList data type"));
//                     }
//                 } else {
//                     st.serialize_field("child_type", &DataType::Null.to_string())?;
//                     st.serialize_field("value", &Option::<Vec<Option<ScalarValue>>>::None)?;
...
//                         st.serialize_field("value", &nested_values)?;
//                     } else {
//                         return Err(serde::ser::Error::custom("Invalid FixedSizeList data type"));
//                     }
//                 } else {
//                     st.serialize_field("child_type", &DataType::Null.to_string())?;
